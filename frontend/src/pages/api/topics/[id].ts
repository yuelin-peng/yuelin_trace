...
async function proxyRequest(req: NextApiRequest, res: NextApiResponse, method: string, endpoint: string) {
  try {
    ...
    console.error(`Topic proxy error (${method} ${endpoint}):`, error);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    return proxyRequest(req, res, 'GET', `/topics/${id}`);
  }

  if (req.method === 'PATCH') {
    return proxyRequest(req, res, 'PATCH', `/topics/${id}`);
  }

  if (req.method === 'DELETE') {
    return proxyRequest(req, res, 'DELETE', `/topics/${id}`);
  }
