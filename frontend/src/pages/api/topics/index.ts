...
async function proxyRequest(req: NextApiRequest, res: NextApiResponse, method: string, endpoint: string) {
  try {
    ...
    console.error(`Topics proxy error (${method} ${endpoint}):`, error);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    ...
    return proxyRequest(req, res, 'GET', `/topics${queryString ? `?${queryString}` : ''}`);
  }

  if (req.method === 'POST') {
    return proxyRequest(req, res, 'POST', '/topics');
  }
