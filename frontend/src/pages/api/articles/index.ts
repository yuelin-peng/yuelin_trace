...
async function proxyRequest(req: NextApiRequest, res: NextApiResponse, method: string, endpoint: string) {
  try {
    ...
    console.error(`Articles proxy error (${method} ${endpoint}):`, error);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    // List articles with query params
    ...
    return proxyRequest(req, res, 'GET', `/articles${queryString ? `?${queryString}` : ''}`);
  }

  if (req.method === 'POST') {
    return proxyRequest(req, res, 'POST', '/articles');
  }
